...
//Creates a branch.
Tree *create_node(char *name, char* author, int word_count, unsigned int UID)
{
	...
	if (node) //if malloc is successful
	{
		...
		size_t author_len = strlen(author) + 1;			 // get string length
		node->author = calloc(author_len, sizeof(char)); // assign memory which is given 0 value
		if (node->author)
		{
...
void insert_tree(Tree **r, char *name,char* author, int word_count, unsigned int UID)
{
	if (*r == NULL)
	{ //if tree is null, create node from data
		*r = create_node(name, author, word_count, UID);
		return;
	}
	if (UID < (*r)->UID) // compare value of UID
	{
		insert_tree(&(*r)->left, name, author, word_count, UID);
	}
	else
	{
		insert_tree(&(*r)->right, name, author, word_count, UID);
	}
}
...
int bstdb_add(char *name, int word_count, char* author)
{
	...
	insert_tree(&root, name, author, word_count, UID);
	if (root)
	{
		return UID;
	}
